...
public class Game  {
	...
	private Round round;
	private TableView<ObservableList<String>> scoresTable;
	...
	public Game(String person1, String person2, String person3, String person4) {
		...
		TableColumn player1Column = new TableColumn(person1);
		TableColumn player2Column = new TableColumn(person2);
		TableColumn player3Column = new TableColumn(person3);
		TableColumn player4Column = new TableColumn(person4);
	...
	public void showRoundResultsAndRestart() {
        ...
        data.add(0, "Round" + this.roundCounter);
        for (int i=1; i<5; i++) {
        	data.add(i, String.valueOf((this.players.get(i).getRoundScore())));
        }
        this.scoresTable.getItems().add(data);
		
		//update totals
		for (int i=0;i<4;i++) {
			this.players.get(i).increaseScore(this.players.get(i).getRoundScore());
	...
	*/
	...
	class Round {
		...
		public Hand cardsLeftInPlay() {
			...
			//fill Hand
			for (int suit=0;suit<Constants.suitsInTheDeck;suit++) {
		...
		class Trick {
			...
			public Card computerMove(Hand h) {
				...
				if (this.cardsInTheTrick.isEmpty()) {
					...
					for (int i=0; i<4; i++) {
						...
						System.out.println(Constants.suit[i]+": "+h.getAverageHasSuit(i, Round.this.cardsLeftInPlay())+", "+h.getChanceForAllCardsComingIntoPlay(i, Round.this.cardsLeftInPlay()));
					}
			...
			public Card humanMove(Hand h) {
				...
				//this doesn't work, but we don't have humans yet.
				return h.getCard(0);
